...
object OpenSessionManager {
    ...
    private const val KEY_DISABLED_TAB_IDS_PREFIX = "disabledTabIds:"
    ...
    fun disabledTabIds(): Set<String> {
        val rawValue = preferences.getString(disabledTabIdsKey(), "") ?: ""
        return rawValue.split(separator).filter { it.isNotBlank() }.toSet()
    }

    fun markTabDisabled(tabId: String) {
        if (tabId.isBlank()) {
            return
        }
        saveDisabledTabIds(disabledTabIds() + tabId)
    }

    fun markTabEnabled(tabId: String) {
        if (tabId.isBlank()) {
            return
        }
        saveDisabledTabIds(disabledTabIds() - tabId)
    ...
    private fun saveDisabledTabIds(tabIds: Set<String>) {
        preferences.edit {
            putString(
                disabledTabIdsKey(),
                tabIds.filter { it.isNotBlank() }.sorted().joinToString(separator = separator)
            )
        }
    }

    private fun disabledTabIdsKey(): String {
        val owner = userId.ifBlank { lastAccount }.ifBlank { "anonymous" }
        return "$KEY_DISABLED_TAB_IDS_PREFIX$owner"
    }
